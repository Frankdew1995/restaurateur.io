...
function typeFilter() {
    ...
    //console.log(type);
    if(type==="water" || children==="single"){//jxy
...
function sendOrder() {
    ...
    $.ajax(
        {
            url:"../php/menuDataByXSTAR.php?q=addDishIntoCertainTable2",
            data:{
            ...
            success:function (res) {
                console.log(res);
                if(res=="good"){
                    window.location.href="thanks2.html"+param;
                }
            },
...
function newItem(item) {
   // console.log(item);
    ...
    item.discibe=item.discibe?item.discibe:String.Current_No_Discribe;
    row.innerHTML="   <div onclick='showDetail(this)' data-dish-id='"+item.dishId+"' class=\"items\">\n" +
        "            <div class=\"itemImg\">\n" +
        "                <img src=\"../resource/image/dishImages/"+item.dishId+".jpg"+"\">\n" +
        "            </div>\n" +
